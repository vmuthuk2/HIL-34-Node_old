...
void VVCAgent::vvc_slave()
{
  ...
  if (status == true)
  {
	  ...
	  sstA_cmd = mean(mean(Grad_slave1.rows(5,6))); 
	  sstB_cmd = 0; //No Phase B loads in Area 2
	  sstC_cmd = 0; //No Phase C loads in Area 3
  }
  else
  {
  ...
  std::cout << "SSTs under Slave #2: SST3 SST4\n" << endl;
