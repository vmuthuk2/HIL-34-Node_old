...
void VVCAgent::vvc_slave()
{
  ...
  if (status == true)
  {
	  ...
	  //tempA = join_rows(Grad_slave1.row(1),Grad_slave1.row(7));
	  //tempA = join_rows(tempA,Grad_slave1.row(8));
	  //std::cout << tempA << std::endl;
	  sstA_cmd = mean(mean(join_cols(Grad_slave1.row(1),Grad_slave1.rows(7,8))));
	  //tempB = join_rows(Grad_slave1.rows(15,18),Grad_slave1.rows(22,25));
	  sstB_cmd = mean(mean(join_cols(Grad_slave1.row(15),Grad_slave1.row(23))));
	  sstC_cmd = mean(mean(join_cols(Grad_slave1.row(31),Grad_slave1.row(35))));
  }
  else
  {
  ...
  std::cout << "SSTs under Slave #5: SST10 SST11 SST12\n" << endl;
