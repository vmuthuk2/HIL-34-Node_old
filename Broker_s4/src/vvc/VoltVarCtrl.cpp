...
void VVCAgent::vvc_slave()
{
  ...
  if (status == true)
  {
	  ...
	  sstB_cmd = mean(mean(join_cols(Grad_slave1.row(14),Grad_slave1.row(22))));
	  sstC_cmd = mean(mean(join_cols(Grad_slave1.row(29),Grad_slave1.row(30))));
  }
  else
  {
  ...
  std::cout << "SSTs under Slave #4: SST7 SST8 SST9 \n" << endl;
