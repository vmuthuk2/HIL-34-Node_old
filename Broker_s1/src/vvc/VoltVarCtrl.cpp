...
void VVCAgent::vvc_slave()
{
  ...
  if (status == true)
  {
	  ...
	  sstA_cmd = 0; //No Phase A loads in Area 1
	  //tempB = join_rows(Grad_slave1.row(12),Grad_slave1.row(20));
	  sstB_cmd = mean(mean(join_cols(Grad_slave1.row(12),Grad_slave1.row(20))));
	  sstC_cmd = mean(mean(Grad_slave1.row(28)));
  }
  else
  {
